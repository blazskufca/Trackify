...
class BigBangTracker:
    def scrape(self, url):
        ...
        try:
            ...
            price = parse_decimal(soup.select_one('.cd-current-price').contents[0].getText().strip().replace(' €', ''),
                                  locale='sl_SI')
            ...
            models.TrackedProducts(name=title, link=response.url, price=price, date=date.today(),
                                   priceHistory=json.dumps(JSON, default=str), description=description).save()
        except:
            pass
...
class AmazonDeTracker:
    ...
    'Mozilla/5.0 (X11; CrOS x86_64 8172.45.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.64 Safari/537.36',
        ...
        try:
            ...
            temp = soup.select_one(".a-offscreen").getText().replace("€", "").replace(',', '')
            price = float(temp)
            ...
            models.TrackedProducts(name=title, link=response.url, price=price, date=date.today(),
                                   priceHistory=json.dumps(JSON, default=str)).save()
        except:
            pass
...
class EnaATracker:
    def scrape(self, url):
        ...
        try:
            soup = BeautifulSoup(response.content, 'html.parser')
            price = parse_decimal(soup.select_one('.single-product-price').getText().strip().replace(' €', ''),
                                  locale='sl_SI')
            title = soup.select_one('.section-title').getText().strip()
            description = soup.select_one('.single-product-description').getText().strip().replace("   Celoten opis",
                                                                                                   "")
            JSON = {
                str(date.today()): price
            }
...
class FuntechTracker:
    def scrape(self, url):
        ...
        try:
            soup = BeautifulSoup(response.content, 'html.parser')
            price = parse_decimal(
                soup.select_one('.nasa_cena').getText().strip().replace('Naša cena:  ', '').replace(' €', ''),
                locale='sl_SI')
